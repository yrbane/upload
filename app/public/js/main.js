...
['dragenter', 'dragover', 'dragleave', 'drop']
    .forEach(e => dropArea.addEventListener(e, ev => {
        ev.preventDefault();
        ev.stopPropagation();
    }, false));
...
if (languageSelect) {
    ...
    function updateFlag(selectedLocale) {
        // Clear existing flag classes
        languageSelect.className = languageSelect.className.replace(/\bflag-icon-\S+/g, '');

        const selectedOption = languageSelect.querySelector(`option[value="${selectedLocale}"]`);
        if (selectedOption && selectedOption.dataset.icon) {
            languageSelect.classList.add(selectedOption.dataset.icon);
        }
    }

    if (currentLocale) {
        languageSelect.value = currentLocale;
        updateFlag(currentLocale);
    }
